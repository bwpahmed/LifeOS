...
export default function AutomationsPage(){
 ...
 async function remove(r:Rule){if(!confirm(`Delete automation "${r.name}"?`))return;const sb=supabaseBrowser();const{error:q}=await sb.from("automation_rules").delete().eq("id",r.id);if(q)setError(q.message);else await load();}
 ...
 {!workspaceId?<p className="panel mt-4 p-5 text-sm">Sign in first. <Link href="/login" className="text-[#8ab6ff]">Login →</Link></p>:<div className="mt-4 grid gap-4 md:grid-cols-[.75fr_1.25fr]">
  ...
  <Panel title="Rules" kicker="RUNS ONLY ENABLED RULES"><div className="space-y-2">{rules.map(r=><div key={r.id} className="flex items-center justify-between gap-3 rounded-lg border border-white/10 p-3"><div><b>{r.name}</b><p className="text-xs text-slate-500">{r.trigger} → {r.action}</p></div><div className="flex gap-1"><button onClick={()=>toggle(r)} className={`rounded-lg px-2 py-1 text-xs ${r.enabled?"bg-emerald-300/20 text-emerald-200":"border border-white/10"}`}>{r.enabled?"Enabled":"Disabled"}</button><button onClick={()=>remove(r)} className="rounded-lg border border-red-400/20 px-2 py-1 text-xs text-red-300">Delete</button></div></div>)}</div><button onClick={run} disabled={!rules.some(r=>r.enabled)} className="mt-4 w-full rounded-lg border border-white/10 p-2 font-bold disabled:opacity-40">Run enabled automations now</button></Panel>
 </div>}
 </main>;
}
