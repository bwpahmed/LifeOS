...
const featureLinks=[
 ...
 ["Money","/money"],["Business","/business"],["Health","/health"],["Family","/family"],["Europe","/europe"],
 ["Calendar","/calendar"],["Timeline","/timeline"],["Automations","/automations"],["Reviews","/reviews"],
 ["Journal","/journal"],["Sticky Notes","/sticky-notes"],["Habits","/habits"],["Focus","/focus"],
 ...
 ["Privacy / PIN","/privacy"],["Access","/access"],["Search","/search"],["Quick Add","/quick-add"]
];
...
export default function SettingsPage(){
 ...
 const[theme,setTheme]=useState<"dark"|"light">("dark");const[google,setGoogle]=useState<GoogleStatus>({authenticated:false,configured:false,connected:false});const[counts,setCounts]=useState({tasks:0,receivables:0,activity:0,notes:0});
 ...
 useEffect(()=>{(async()=>{try{
   ...
   const[p,n,t,r,a,s,g]=await Promise.all([
     sb.from("profiles").select("timezone,currency").eq("id",ctx.user.id).maybeSingle(),
   ...
   if(p.data){setTimezone(p.data.timezone||"Asia/Dubai");setCurrency(p.data.currency||"AED");}
   ...
   setCounts({tasks:t.count||0,receivables:r.count||0,activity:a.count||0,notes:s.count||0});setGoogle(g as GoogleStatus);
