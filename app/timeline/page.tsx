...
export default function TimelinePage(){
 ...
 const load=useCallback(async()=>{try{const sb=supabaseBrowser();const ctx=await currentWorkspace(sb);if(!ctx){setWorkspaceId("");return;}setWorkspaceId(ctx.workspaceId);
  const[activity,focus,journal,receivables,expenses,water,sleep,baby,time]=await Promise.all([
   ...
   sb.from("money_expenses").select("id,date,amount,category,merchant,is_waste,created_at").eq("workspace_id",ctx.workspaceId).order("created_at",{ascending:false}).limit(100),
   sb.from("water_logs").select("id,date,amount_ml,created_at").eq("workspace_id",ctx.workspaceId).order("created_at",{ascending:false}).limit(100),
   sb.from("sleep_sessions").select("id,date,duration_min,quality,created_at").eq("workspace_id",ctx.workspaceId).order("created_at",{ascending:false}).limit(100),
   sb.from("baby_records").select("id,date,type,title,value,created_at").eq("workspace_id",ctx.workspaceId).order("created_at",{ascending:false}).limit(100),
   sb.from("time_entries").select("id,date,category,minutes,note,created_at").eq("workspace_id",ctx.workspaceId).order("created_at",{ascending:false}).limit(100)
  ]);for(const q of[activity,focus,journal,receivables,expenses,water,sleep,baby,time])if(q.error)throw q.error;const recs=receivables.data||[];const ids=recs.map((r:any)=>r.id);
  ...
  const all:Row[]=[
   ...
   ...(expenses.data||[]).map((x:any)=>({id:`e-${x.id}`,at:x.created_at,kind:x.is_waste?"Waste":"Expense",title:`AED ${Number(x.amount||0).toLocaleString()} · ${x.category}`,detail:x.merchant||""})),
   ...(water.data||[]).map((x:any)=>({id:`w-${x.id}`,at:x.created_at,kind:"Health",title:`Water ${x.amount_ml} ml`,detail:x.date})),
   ...(sleep.data||[]).map((x:any)=>({id:`s-${x.id}`,at:x.created_at,kind:"Sleep",title:`${Math.round(Number(x.duration_min||0)/6)/10}h sleep`,detail:`${x.date} · quality ${x.quality??"—"}/10`})),
   ...(baby.data||[]).map((x:any)=>({id:`b-${x.id}`,at:x.created_at,kind:"Baby",title:x.title,detail:`${x.date} · ${x.type||""}${x.value?" · "+x.value:""}`})),
   ...(time.data||[]).map((x:any)=>({id:`t-${x.id}`,at:x.created_at,kind:"Time",title:`${x.category} · ${x.minutes} min`,detail:x.note||x.date}))
