...
export default function TodayPage(){
 const[tasks,setTasks]=useState<TaskRow[]>([]);const[signedIn,setSignedIn]=useState<boolean|null>(null);const[error,setError]=useState("");const[msg,setMsg]=useState("");const[offline,setOffline]=useState(false);
 ...
 async function morningCheckIn(){const energyRaw=prompt("Morning energy 1–10","7");if(energyRaw==null)return;const energy=Math.max(1,Math.min(10,Number(energyRaw)));if(!Number.isFinite(energy))return;const moodRaw=prompt("Mood 1–10","7");if(moodRaw==null)return;const mood=Math.max(1,Math.min(10,Number(moodRaw)));if(!Number.isFinite(mood))return;try{const sb=supabaseBrowser();const ctx=await currentWorkspace(sb);if(!ctx)throw new Error("Sign in first.");const date=todayInTZ();const existing=await sb.from("health_entries").select("id").eq("workspace_id",ctx.workspaceId).eq("date",date).maybeSingle();const payload={workspace_id:ctx.workspaceId,created_by:ctx.user.id,date,energy,mood};const q=existing.data?await sb.from("health_entries").update({energy,mood}).eq("id",existing.data.id):await sb.from("health_entries").insert(payload);if(q.error)throw q.error;setMsg("Morning check-in saved.");}catch(e){setError(e instanceof Error?e.message:"Could not save check-in");}}
