...
export async function POST(request:Request){
 const client=await supabaseServer();const{data:auth,error:authError}=await client.auth.getUser();if(authError||!auth.user?.email)return NextResponse.json({error:"Sign in first"},{status:401});
