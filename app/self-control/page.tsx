...
export default function SelfControlPage(){
  ...
  const insight=useMemo(()=>{const counts=(key:"trigger"|"response")=>rows.reduce<Record<string,number>>((a,r)=>{const v=String(r[key]||"Unknown");a[v]=(a[v]||0)+1;return a;},{});const top=(o:Record<string,number>)=>Object.entries(o).sort((a,b)=>b[1]-a[1])[0]?.[0]||"—";const hours=rows.map(r=>Number((r.time||"0").split(":")[0])).filter(Number.isFinite);const avg=hours.length?Math.round(hours.reduce((a,b)=>a+b,0)/hours.length):null;const controlled=rows.filter(r=>r.outcome==="Controlled").length;return{topTrigger:top(counts("trigger")),topResponse:top(counts("response")),avgHour:avg,success:rows.length?Math.round(controlled/rows.length*100):0};},[rows]);
  ...
  return <main className="pt-6"><BackHome/><p className="mt-2 text-[11px] tracking-widest text-slate-400">SELF-CONTROL</p><h1 className="text-2xl font-bold">Track triggers, not shame</h1>{error&&<p className="mt-3 text-sm text-red-300">{error}</p>}
    {!workspaceId?<p className="panel mt-4 p-5 text-sm">Sign in first. <Link href="/login" className="text-[#8ab6ff]">Login →</Link></p>:<div className="mt-4 grid gap-4 md:grid-cols-2">
      ...
      <Panel title="Patterns" kicker="RECENT LOGS"><div className="grid grid-cols-2 gap-2 text-sm"><div className="rounded-lg border border-white/10 p-3"><span className="text-xs text-slate-500">Top trigger</span><b className="block">{insight.topTrigger}</b></div><div className="rounded-lg border border-white/10 p-3"><span className="text-xs text-slate-500">Best response</span><b className="block">{insight.topResponse}</b></div><div className="rounded-lg border border-white/10 p-3"><span className="text-xs text-slate-500">Control rate</span><b className="block">{insight.success}%</b></div><div className="rounded-lg border border-white/10 p-3"><span className="text-xs text-slate-500">Typical hour</span><b className="block">{insight.avgHour==null?"—":`${insight.avgHour}:00`}</b></div></div>
