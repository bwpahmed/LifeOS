...
All flags forward through `"$(git rev-parse --show-toplevel)/.agents/vendor/claude-seo/scripts/claude-seo" run unlighthouse_run.py`, which handles
...
- For LCP subpart decomposition on slow pages, use the
  `"$(git rev-parse --show-toplevel)/.agents/vendor/claude-seo/scripts/claude-seo" run lcp_subparts.py` workflow (Phase C).
