...
instead of WebSearch for SERP data. Run `"$(git rev-parse --show-toplevel)/.agents/vendor/claude-seo/scripts/claude-seo" run dataforseo_costs.py check serp_organic_live_advanced --count N`
...
- All URLs fetched via `"$(git rev-parse --show-toplevel)/.agents/vendor/claude-seo/scripts/claude-seo" run render_page.py <url> --mode auto` (SPA-aware SSRF protection via `url_safety`)
