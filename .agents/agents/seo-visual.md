...
```bash
"$(git rev-parse --show-toplevel)/.agents/vendor/claude-seo/scripts/claude-seo" run capture_screenshot.py URL --all --output screenshots/
"$(git rev-parse --show-toplevel)/.agents/vendor/claude-seo/scripts/claude-seo" run render_page.py URL --mode auto --a11y-tree --json
```
