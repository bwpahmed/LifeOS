...
```
"$(git rev-parse --show-toplevel)/.agents/vendor/claude-seo/scripts/claude-seo" run metadata_template.py --title "<title>" --description "<desc>" --json
```
...
```
"$(git rev-parse --show-toplevel)/.agents/vendor/claude-seo/scripts/claude-seo" run metadata_template.py --pairs-file metadata.json --json
```
